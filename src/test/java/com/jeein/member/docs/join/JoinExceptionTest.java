...
import static com.jeein.member.docs.snippets.CommonSnippet.CommonDescriptor.errorCodeOnlyResponseFields;
...
public class JoinExceptionTest {
    ...
    void joinMember_emailAlreadyExists() throws Exception {
        JoinRequestDTO duplicateRequest = JoinRequestDTO.of("email@example.com", "새 이름", "새 닉네임", "password");
        ...
        mockMvc.perform(post(ApiPath.MEMBER_JOIN)
                ...
                .andDo(doc(JOIN_EXCEPTION_DUPLICATION_EMAIL, MEMBER_JOIN_REQUEST_FIELDS, errorCodeOnlyResponseFields()));
    }

    @Test
    @DisplayName("회원가입 요청 닉네임이 이미 존재하면 409 에러와 메시지를 반환한다.")
    void joinMember_nicknameAlreadyExists() throws Exception {
        JoinRequestDTO duplicateRequest = JoinRequestDTO.of("another@example.com", "이름", "닉네임", "password");
        ...
        mockMvc.perform(post(ApiPath.MEMBER_JOIN)
                ...
                .andExpect(jsonPath("$.errors").doesNotExist())
                .andExpect(jsonPath("$.data").doesNotExist())
                .andDo(doc(JOIN_EXCEPTION_DUPLICATION_NICKNAME, MEMBER_JOIN_REQUEST_FIELDS, errorCodeOnlyResponseFields()));
    }
}
