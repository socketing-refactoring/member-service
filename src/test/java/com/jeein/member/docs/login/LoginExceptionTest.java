...
public class LoginExceptionTest {
    ...
    void login_withInvalidEmail_shouldReturnForbidden() throws Exception {
        ...
        mockMvc.perform(post(ApiPath.MEMBER_LOGIN).content(objectMapper.writeValueAsString(loginRequest)))
                .andExpect(status().isForbidden())
                .andExpect(jsonPath("$.code").value(errorCode.getCode()))
                .andExpect(jsonPath("$.message").value(errorCode.getMessage()))
                .andExpect(jsonPath("$.errors").doesNotExist())
                .andExpect(jsonPath("$.data").doesNotExist())
                .andDo(doc(LOGIN_EXCEPTION_INVALID_EMAIL, MEMBER_LOGIN_REQUEST_FIELDS, errorCodeOnlyResponseFields()));
    }
    ...
    void login_withInvalidPassword_shouldReturnForbidden() throws Exception {
        ...
        mockMvc.perform(post(ApiPath.MEMBER_LOGIN).content(objectMapper.writeValueAsString(loginRequest)))
                ...
                .andExpect(jsonPath("$.errors").doesNotExist())
                .andExpect(jsonPath("$.data").doesNotExist())
                .andDo(doc(LOGIN_EXCEPTION_INVALID_PASSWORD, MEMBER_LOGIN_REQUEST_FIELDS, errorCodeOnlyResponseFields()));
    }
}
